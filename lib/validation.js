...
var validateAccess = function(props, schema, i) {
  ...
  // square brackets object access
  if (schemaProp.computed) {
    if (schema.title !== 'number') {
      return null;
    } else {
      return props.join('.');
    }
  ...
  if (schema.$ref) {
    schema = accessor.getSchemaByRef(schema.$ref);
  }
...
var validate = function (param, scope, node, context) {
  ...
  if (param.unions) {
    ...
    schema = accessor.getUnionSchema(param.unions, param.definedTypes);
  } else if (param.varDefinedType) {
    schema = param.varDefinedType;
    ...
    if (!schema) {
      schema = accessor.getSchemaByType(param.varType, param.definedTypes);
    }
  } else {
