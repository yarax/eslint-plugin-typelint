...
function getSchemaByType(type) {
  ...
  if (type.match(/\./)) {
    ...
    if (schemaObj.items) schemaObj = schemaObj.items;
    return schemaObj;
  }
...
function validateBySchema(param, scope, node, context, native) {
  ...
  try {
    schema = native ? getNativeSchema(param.varType) : getSchemaByType(param.varType);
  } catch (e) {
  ...
  valid = validateAccess(scope.props, schema, 0, !native);
  if (valid !== null) {
